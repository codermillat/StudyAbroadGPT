...
\begin{sloppypar}
We \textbf{pre-registered} a stratified method-vs-data test (protocol: \path{docs/analysis-plans/2026-09-02-stratified-causal-test.md}, frozen 2026-09-02; commit \texttt{e86405b}, full SHA \texttt{e86405b711d75972ec798b12cfcfbfe121870993}, committed 2026-09-02 22:32:03 IST).\footnote{The git commit timestamp on the pre-registration file is the binding pre-registration time; the supplementary repository's git history preserves this timestamp immutably and is verifiable after unblinding. For double-blind integrity, the supplementary will additionally publish the pre-registration SHA in a separate, anonymous Gist at submission time so the timestamp can be confirmed by reviewers before unblinding.} Each held-out prompt is assigned to \textbf{C} ($\ge$3 verified-correct training neighbors), \textbf{W} ($\ge$3 verified-wrong), or \textbf{M} (mixed); \textbf{McNemar's test} is run on the paired base/LoRA 2$\times$2 within each stratum. \textbf{§6.4 rule}: LoRA error $>$ base \emph{only} in W $\Rightarrow$ \emph{established}; both strata $\Rightarrow$ ``uniformly worse.'' We extended the v4 source-verified training audit from 40 to 149 entries (43 verified-wrong); of the 50 v4 held-out prompts, 15/14/1/20 fall in C/W/M/excluded. We re-used the v4 base/LoRA outputs and source-verified every specific claim in the 60 stratified outputs; 2 of 60 were \texttt{unclear}, leaving 28 analyzable pairs.
\end{sloppypar}
