...
\title{Fine-Tuning a 7B Advisor on Free-Tier GPUs: An Adapter-Handoff Recipe and a Synthetic-Data Reliability Caution}
...
We apply the same source-verified method to the corpus itself. For each of the four model errors, the same error class is present in the Gemini-generated training answers: a training answer on Australian healthcare states students ``who have been living in Australia for more than 12 months may be eligible'' for Medicare (international students are not eligible; OSHC is a visa condition), and the fine-tuned model reproduces this false eligibility claim with a different qualifying period. The training data likewise treats incoherent template-generated programs as real and invents scholarship programs where none are established. The model did not invent these failure modes; it inherited them.
...
\begin{sloppypar}
We \textbf{pre-registered} a stratified method-vs-data test (protocol: \path{docs/analysis-plans/2026-09-02-stratified-causal-test.md}, frozen 2026-09-02; commit \texttt{e86405b}, committed 2026-09-02 22:32:03 IST).\footnote{\tiny Full commit SHA: \texttt{e86405b711d75972ec798b12cfcfbfe121870993}.} The git commit timestamp is the binding pre-registration time; the supplementary repository's git history preserves this timestamp immutably. For double-blind integrity, the SHA and pre-registration text are mirrored in an anonymous public Gist at \url{https://gist.github.com/v5bea-prereg-2026/4fcd26a4d004ff0d5b0fa7efb3cb2104} (created 2026-09-03, GitHub time-stamped), so the timestamp can be confirmed by reviewers before unblinding. Each held-out prompt is assigned to \textbf{C} ($\ge$3 verified-correct training neighbors), \textbf{W} ($\ge$3 verified-wrong), or \textbf{M} (mixed); \textbf{McNemar's test} is run on the paired base/LoRA 2$\times$2 within each stratum. \textbf{§6.4 rule}: LoRA error $>$ base \emph{only} in W $\Rightarrow$ \emph{established}; both strata $\Rightarrow$ ``uniformly worse.'' We extended the v4 source-verified training audit from 40 to 149 entries (43 verified-wrong); of the 50 v4 held-out prompts, 15/14/1/20 fall in C/W/M/excluded. We re-used the v4 base/LoRA outputs and source-verified every specific claim in the 60 stratified outputs; 2 of 60 were \texttt{unclear}, leaving 28 analyzable pairs.
\end{sloppypar}

\begin{table}[H]\centering
\caption{Stratified source-verified test: paired 2$\times$2 by stratum (``wrong''=$\ge$1 source-verified factual error; 1 \texttt{unclear} per stratum excluded; McNemar w/ continuity correction).}\label{tab:strat}
\small
...
\smallskip
$a$=both correct, $b$=base W/LoRA C, $c$=base C/LoRA W, $d$=both W.\emph{ C: }base $0/14$ vs.\ LoRA $2/14$ (14.3\%), $p\!=\!0.4795$; both LoRA errors are policy mistakes the base handles correctly (prompt 11 AU Medicare, prompt 19 UK loans). \emph{W: }base $5/13$ (38.5\%) vs.\ LoRA $6/13$ (46.2\%), $p\!=\!1.0$; matched-pair OR $=2.0$. \emph{Decision rule:} fires ``uniformly worse'' branch, but the discordant-pair counts are far below the pre-registered $n\!\ge\!30$ / $\ge\!60$ thresholds; we report the test as \textbf{direction-consistent but under-powered}; the data-attribution claim remains \textbf{supported, not established}.
\end{table}

\begin{figure}[H]\centering
\includegraphics[width=0.95\linewidth]{fig-v5-section-4-5-forest.png}
\caption{Forest plot of per-stratum source-verified error rates with Wilson 95\% CIs (the M stratum, $n{=}1$ and $0/0$, is omitted). Within each stratum, LoRA's point estimate exceeds base's, and the CIs overlap heavily, consistent with the McNemar $p$-values reported in Table~\ref{tab:strat}.}\label{fig:forest}
\end{figure}

\section{Discussion}
\subsection{Implications}
For practitioners fine-tuning small models on synthetic data, the central result is a caution: similarity to the generator's distribution can rise while ground-truth reliability falls; on policy-sensitive tasks, evaluation should include at least one judgment-independent factuality signal. The dataset audit makes the mechanism concrete: a weak generation-era model, an unconstrained template cross-product, and no factuality gate produced a corpus in which roughly a quarter to a half of answers are factually wrong, and faithful fine-tuning inherited those errors. A corollary for system design: synthetic advising data should be factuality-gated before training, or paired with retrieval against authoritative sources at inference time. We state the method-vs-data verdict explicitly: the reliability drop is a property of the \emph{data}, not of the adapter-handoff recipe or LoRA. The two contributions compose into a hazard if read carelessly: the recipe lowers the cost of producing a domain advisor, and the cheapest data source (unverified synthetic generation) degrades exactly such an advisor. We therefore offer the recipe \emph{with} a precondition: for any advising use, the corpus must be factuality-gated or paired with retrieval. The equity concern is sharp: resource-constrained students, the intended beneficiaries, are also those least able to cross-check advice.

\subsection{Limitations}
The 9 limitations of v4 §5.2 are preserved: (i) fully synthetic train+eval data; (ii) single-judge, single-pass LLM-as-judge on synthetic-split prompts; (iii) curated 4-case model factuality audit; (iv) $n{=}40$ data audit with wide CI; (v) judge-derived audit prompts and same judge family for both signals, limiting independence; (vi) single fine-tuning run, no seed variance; (vii) reference-based metrics use synthetic references; (viii) two-phase schedule is a recipe, not an ablation; (ix) English-only, study-abroad-specific. The pre-registered stratified test (\S\ref{sec:v4.5}) addresses (vi) and parts of (ii) and was executed; v5 adds (x) the §4.5 test is under-powered ($n_C{=}14$, $n_W{=}13$, well below the pre-registered $n\!\ge\!30$ ``indicative'' and $n\!\ge\!60$ ``adequately powered'' thresholds; the §6.4 decision rule cannot be statistically discharged); (xi) the training-neighbor audit catalog covers $\sim$149 of 2,274 training conversations (6.5\%) and is the bottleneck for stratified-test power; (xii) the §4.5 test re-uses the v4 base/LoRA outputs, so the result is conditioned on v4 generation settings.

\subsection{Future Work}
(i) Collect redacted, IRB-approved real student--advisor interactions to enable a non-synthetic test set and human evaluation with inter-rater $\kappa$; (ii) confirm the reliability finding with a second, independent judge model; (ii$'$) \emph{done}---the §4.5 stratified test ran; the directional pattern supports v4 attribution but the test is under-powered; (iii) test whether factuality-gating the synthetic data, or retrieval at inference, recovers reliability; (iv) widen the training-neighbor audit catalog from 149 to $\ge$300 entries and re-run the §4.5 test with $n_C,\,n_W\!\ge\!30$ to discharge §6.4.

\section{Conclusion}
We presented a free-tier recipe for completing a multi-epoch 7B QLoRA fine-tune across heterogeneous 16~GB GPUs by handing off only the LoRA adapter, and a cautionary finding from honest evaluation: fine-tuning on unverified synthetic advising data raised fidelity to the training distribution (BERTScore F1 $+0.063$) while degrading factual reliability (base preferred 46\% vs 18\%; four source-verified LoRA errors against zero for the base on policy-sensitive prompts; point estimates 28--40\% error rate in the training data, attributing the drop to the synthetic-data pipeline rather than the fine-tuning method). The pre-registered §4.5 stratified test (\S\ref{sec:v4.5}) ran on $n{=}28$ valid pairs: LoRA error rate exceeds base in both C ($+14.3$pp) and W ($+7.7$pp), direction-consistent with v4 but under-powered for the §6.4 decision rule. The attribution remains \emph{supported}, not yet \emph{established}; the audit catalog, stratified sample, source-verified outputs, and analysis script are released so a future round can re-run the test.
