...
LLMs demonstrate broad task generalization~\cite{brown2020,jiang2023,zhao2023} but lack domain specificity and can produce fluent but incorrect answers on policy-sensitive topics~\cite{ippolito2020}. LoRA~\cite{hu2021} and QLoRA~\cite{dettmers2023} enable parameter-efficient adaptation on consumer hardware, with kernel-level support from Unsloth~\cite{unsloth2024}; GPTQ~\cite{frantar2022} addresses inference-time quantization. Multi-session resumption is standard via the Hugging Face Trainer~\cite{wolf2020} and the PEFT adapter interface~\cite{peft2022}; this work uses \emph{only} that adapter interface across the GPU boundary. Synthetic instruction data~\cite{wang2022} is now common, but carries documented risks: model collapse under recursive training~\cite{shumailov2024}, quality degradation in self-consuming loops~\cite{alemohammad2023}, imitation of style but not factuality~\cite{gudibande2023}, and increased hallucination when fine-tuning introduces knowledge a model did not consolidate in pre-training~\cite{gekhman2024}---precisely the hazard when a comparatively weak generator supplies claims outside its reliable boundary. Two evaluation pitfalls follow: reference-based metrics against \emph{synthetic} references (e.g., BERTScore~\cite{zhang2020}) measure fidelity to the generator's distribution, not ground-truth quality~\cite{ji2023}; and LLM-as-judge~\cite{zheng2023} has known biases, including self-preference~\cite{panickssery2024}, which we mitigate by using a judge from a different model family than the data generator. Educational advising applications have used rule-based and retrieval systems~\cite{cha2024,kaur2022}; LLM-based advising is the natural next step, and factual reliability---not merely fluency---is the property that matters.
...
\begin{tabular}{p{2.2cm}p{4.5cm}p{4.7cm}}
...
Australian healthcare & States students are ``eligible for Medicare after \ldots at least six months.'' & States international students are not eligible; OSHC compulsory. \\
