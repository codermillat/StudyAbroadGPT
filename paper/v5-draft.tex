...
\item A cautionary reliability finding, with the cause isolated to the synthetic-data pipeline. A source-verified audit and a direct audit of the training data establish the same conclusion. We ran the pre-registered stratified test (\S\ref{sec:v4.5}); the directional pattern is consistent with the data-attribution claim but the test is under-powered for the pre-registered decision rule. The attribution remains \emph{supported, not established}; the pre-registration, audit catalog, and analysis script are released so a future round with more labeled prompts can discharge the test.
\end{enumerate}
...
\subsection{Pre-Registered Stratified Causal Test}\label{sec:v4.5}

\begin{sloppypar}
We \textbf{pre-registered} a stratified method-vs-data test (protocol: \path{docs/analysis-plans/2026-09-02-stratified-causal-test.md}, frozen 2026-09-02; commit \texttt{e86405b}). For each held-out prompt, retrieve its top-$k\!=\!5$ training-set nearest neighbors (cosine over \texttt{sentence-transformers/all-MiniLM-L6-v2}), label each neighbor via the source-verified catalog, assign the prompt to \textbf{C} ($\ge$3 verified-correct, 0 verified-wrong), \textbf{W} ($\ge$3 verified-wrong, 0 verified-correct), or \textbf{M} (mixed), then run \textbf{McNemar's test} on the paired base/LoRA 2$\times$2 within each stratum. \textbf{Pre-registered §6.4 rule}: if LoRA error rate exceeds base only in W, attribution is \emph{established}; if both strata show LoRA $>$ base, attribution weakens to ``uniformly worse.''
\end{sloppypar}

\begin{sloppypar}
\textbf{Execution.} We extended the v4 source-verified training audit from 40 to 149 entries (43 verified-wrong). Of the 50 v4 held-out prompts, 15 fall in C, 14 in W, 1 in M, 20 excluded (insufficient audited neighbors). We re-used the v4 base/LoRA outputs and source-verified every specific, falsifiable claim in each of the 60 stratified outputs against authoritative external sources (\texttt{.edu}, \texttt{.gov}, official scholarship/visa pages); 2 of 60 outputs were labeled \texttt{unclear} (1 per stratum), leaving 28 analyzable pairs. Per-prompt labels and source URLs are in \texttt{data/v5-audit-results.csv} and the analysis script is \texttt{scripts/eval/analyze\_stratified.py}.
\end{sloppypar}

\begin{table}[H]\centering
\caption{Stratified source-verified test: paired 2$\times$2 by stratum. ``wrong''=$\ge$1 source-verified factual error; ``correct''=none. Excludes 1 \texttt{unclear} per stratum. McNemar's test is with continuity correction.}\label{tab:strat}
\small
\begin{tabular}{lrrrrrr}
\hline\textbf{Stratum} & $n$ & $a$ & $b$ & $c$ & $d$ & \textbf{McNemar $p$} \\
\hline
C (clean)    & 14 & 12 & 0  & 2 & 0  & 0.4795 \\
W (wrong)    & 13 &  6 & 1  & 2 & 4  & 1.0000 \\
M (mixed)    &  1 &  1 & 0  & 0 & 0  & n/a    \\
\hline
\end{tabular}

\smallskip
$a$=both correct, $b$=base wrong/LoRA correct, $c$=base correct/LoRA wrong, $d$=both wrong.
\end{table}

\begin{sloppypar}
\textbf{Stratum~C.} Base $0/14$; LoRA $2/14$ (14.3\%); McNemar $p\!=\!0.4795$ (no CC: $p\!=\!0.25$). Both LoRA-wrong cases are policy errors the base handles correctly: prompt~11 (Australian Medicare---the LoRA claims international students become Medicare-eligible after 6~months; OSHC remains compulsory) and prompt~19 (UK student loans---the LoRA claims maintenance loans are non-means-tested, contradicting GOV.UK). \emph{Direction: LoRA worse, both discordant pairs go the LoRA-wrong way.}
\end{sloppypar}

\begin{sloppypar}
\textbf{Stratum~W.} Base $5/13$ (38.5\%); LoRA $6/13$ (46.2\%); McNemar $p\!=\!1.0$ (no CC: $p\!=\!0.564$); matched-pair OR (LoRA-wrong : base-wrong among discordants) $= c/b = 2.0$. \emph{Direction: LoRA worse, smaller effect than in C.}
\end{sloppypar}

\begin{sloppypar}
\textbf{Decision rule (§6.4) outcome.} Mechanically, both strata show LoRA $>$ base, so the rule fires the ``uniformly worse'' branch. We do \emph{not} interpret this strongly: the C-stratum effect is 2/14 prompts and the W-stratum discordant-pair count is 3, so statistical power is far below the pre-registered $n\!\ge\!30$ (indicative) and $n\!\ge\!60$ (adequately powered) thresholds. We report the test as \emph{direction-consistent} (LoRA error rate exceeds base in both strata) but \emph{under-powered} to discharge §6.4. The data-attribution claim remains \textbf{supported, not established}; the audit catalog, stratified sample, source-verified outputs, and analysis script are released so a future round with more labeled prompts can re-run the test. The single highest-leverage follow-up is widening the training-neighbor audit catalog from 149 to $\ge$300 entries.
\end{sloppypar}
...
The 9 limitations of v4 §5.2 are preserved: (i) fully synthetic train+eval data; (ii) single-judge, single-pass LLM-as-judge on synthetic-split prompts; (iii) curated 4-case model factuality audit; (iv) $n{=}40$ data audit with wide CI; (v) judge-derived audit prompts and same judge family for both signals, limiting independence; (vi) single fine-tuning run, no seed variance; (vii) reference-based metrics use synthetic references; (viii) two-phase schedule is a recipe, not an ablation; (ix) English-only, study-abroad-specific. The pre-registered stratified test (\S\ref{sec:v4.5}) addresses (vi) and parts of (ii) and was executed; v5 adds (x) the §4.5 test is under-powered: $n_C{=}14$, $n_W{=}13$ after one \texttt{unclear} exclusion per stratum, well below the pre-registered $n\!\ge\!30$ ``indicative'' threshold and the $n\!\ge\!60$ ``adequately powered'' threshold. The directional pattern is consistent with the v4 data-attribution claim but the §6.4 decision rule cannot be statistically discharged. (xi) The training-neighbor audit catalog is the bottleneck for stratified-test power: it covers $\sim$149 of 2,274 training conversations (6.5\%); widening this catalog is the single highest-leverage follow-up. (xii) The §4.5 test re-uses the v4 base/LoRA outputs, which were generated with the deterministic v4 settings; a fresh generation pass at a higher max-tokens limit or a different sampling strategy was not done, so the §4.5 result is conditioned on v4 generation settings.

\subsection{Future Work}
(i) Collect redacted, IRB-approved real student--advisor interactions to enable a non-synthetic test set and human evaluation with inter-rater $\kappa$; (ii) confirm the reliability finding with a second, independent judge model and a full systematic factuality audit; (ii$'$) \emph{done}---the §4.5 stratified test was run; the directional pattern supports the v4 attribution but is under-powered for the pre-registered decision rule; (iii) test whether factuality-gating the synthetic data, or retrieval against authoritative sources, recovers reliability; (iv) extend the training-neighbor audit catalog from 149 to $\ge$300 entries (and re-stratify the v4 held-out prompts) so a future round can re-run the §4.5 test with $n_C,\,n_W\!\ge\!30$ and discharge the §6.4 decision rule.

\section{Conclusion}
We presented a free-tier recipe for completing a multi-epoch 7B QLoRA fine-tune across heterogeneous 16~GB GPUs by handing off the LoRA adapter only, and a cautionary finding from honest evaluation: fine-tuning on unverified synthetic advising data raised fidelity to the training distribution (BERTScore F1 $+0.063$) while degrading factual reliability (base preferred 46\% vs 18\% by a blind LLM-as-judge; four source-verified LoRA errors against zero for the base on policy-sensitive prompts). The training data, audited with the same source-verified method, is sufficient to account for the failure (point estimates 28--40\% error rate), which attributes the drop to the synthetic-data pipeline rather than the fine-tuning method. The pre-registered §4.5 stratified test (\S\ref{sec:v4.5}) was run on $n{=}28$ valid pairs: LoRA error rate exceeds base in both Stratum~C ($+14.3$pp, $n_C{=}14$) and Stratum~W ($+7.7$pp, $n_W{=}13$); the directional pattern is consistent with the v4 data-attribution claim but the test is under-powered for the pre-registered §6.4 decision rule. The attribution remains \emph{supported}, not yet \emph{established}; the audit catalog, stratified sample, source-verified outputs, and analysis script are released so a future round with more labeled prompts can re-run the test.

\section*{Data and Code Availability}
\begin{sloppypar}
The dataset (2,676 conversations, train + held-out test) is at \texttt{[Anonymized Dataset Hub]} (commit \texttt{3f91dd0}). The LoRA adapter is at \texttt{[Anonymized Model Hub]} (commit \texttt{687758d}). The training pipeline and dataset generation scripts are at \texttt{[Anonymized Repository]}. The supplementary repository releases the evaluation harness (50-prompt blinded base-vs-LoRA outputs, automatic metrics, LLM-as-judge scripts), the source-verified model and dataset factuality catalogs, the v4 40-sample data-audit catalog and its v5 149-row extension (\path{data/v5-audit-catalog.csv}, 43 verified-wrong), the §4.5 stratified test artifacts (\path{data/v5-audit-results.csv}, \path{data/v5-analysis/}, \path{data/v5-stratified-prompts.csv}), the analysis script (\path{scripts/eval/analyze_stratified.py}), the merge helper (\path{scripts/eval/merge_stratified_audits.py}), and the pre-registered protocol (\path{docs/analysis-plans/2026-09-02-stratified-causal-test.md}). These reproduce every result in \S3 and \S4, including the §4.5 stratified test verdict. The HF dataset and model cards carry the v4 factuality finding and a research-only intended-use statement; the released model is not fit for deployment as a student-advising system without prior factuality-gating or retrieval grounding.
\end{sloppypar}
